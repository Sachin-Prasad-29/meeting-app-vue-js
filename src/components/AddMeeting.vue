      ...
      <div class="add-section bg-primary mt-1 mb-3">
        ...
        <form @submit.prevent="onSubmit" id="add-meetings">
          ...
          <div>
            ...
            <div>
              <input type="time" v-model="startTime" id="start-time" name="startTime" class="time-block" />
            </div>
          </div>
          ...
          <div>
            ...
            <div>
              ...
              />
              <div>
                <span>Seprate emailId's/team short names by commas- team short names always begin with @</span>
              </div>
            </div>
          </div>
...
import { meetingMethods } from '@/services/addMeetingMethod';
...
export default {
  ...
  methods: {
    getAllRes() {  // THIS METHOD WILL COLLECT ALL THE RESOURCES REQUIRED FOR THE VALIDATION OF ATTENDEES LIKE ALL THE EMAIL AND TEAM DATA
      //console.log('Hello User');
      this.userArr = this.$store.getters.getAllUsers;
      this.teamArr = this.$store.getters.getAllteamName;
      const emailAndTeam = [];
      this.userArr = this.userArr.split(',');
      this.teamArr = this.teamArr.split(',');
    ...
    async onSubmit() {
      if(this.isValid()){
         ...
         // This part validate the attendees and teams
          const tempArr = this.attendees.split(',');
